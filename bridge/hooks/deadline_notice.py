...
The bands are measured, not picked for roundness. Across the last 12 Nova
cycles that posted a reply (2026-08-10, from Agora's own message
...
- 15 min sits above the worst wrap-up observed, so band 1 is "you can
...
def message_for(band, minutes_left, timeout_minutes):
    if band >= 3:
        return (
            f"TIME NEARLY UP — about {minutes_left:.0f} min before this turn is "
            "killed. Only your reply is still saveable. Post it now, and say "
            "plainly in it what you did and did not finish."
        )
    if band == 2:
        return (
            f"TIME CRITICAL — about {minutes_left:.0f} min left of this turn's "
            f"{timeout_minutes:.0f}-minute limit. Start nothing new." + WRAP_UP
        )
    if band == 1:
        return (
            f"TIME LOW — about {minutes_left:.0f} min left of this turn's "
            f"{timeout_minutes:.0f}-minute limit. Land what you have rather than "
            "opening another thread of work." + WRAP_UP
        )
    return (
        f"Time check: {minutes_left:.0f} of this turn's {timeout_minutes:.0f} "
...
def main():
    ...
    print(json.dumps({"hookSpecificOutput": {
        "hookEventName": event,
        "additionalContext": message_for(band, minutes_left, timeout_minutes),
    }}))
